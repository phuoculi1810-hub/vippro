...
try {
    ...
    console.warn('⚠️ Fetch không kh�?dụng, một s�?tính năng có th�?không hoạt động');
}
...
class KeySystem {
    ...
    isValidKey(keyId, userId) {
        ...
        if (key.type === 'usage' && key.used >= key.value) {
            return { valid: false, reason: 'Key đã hết lượt s�?dụng' };
        }
        ...
        if (key.maxUsers) {
            ...
            if (uniqueUsers >= key.maxUsers && !key.userUsage[userId]) {
                return { valid: false, reason: `Key ch�?cho phép ${key.maxUsers} người dùng` };
            }
        }
...
const commands = [
    new SlashCommandBuilder()
        .setName('test')
        .setDescription('Test command đ�?kiểm tra bot hoạt động'),
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('key')
                .setDescription('Key đ�?s�?dụng lệnh')
                .setRequired(true)),
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('type')
                ...
                .setRequired(true)
                .addChoices(
                    { name: 'Theo s�?lần s�?dụng', value: 'usage' },
                    { name: 'Theo thời gian', value: 'time' }
                ))
        .addIntegerOption(option =>
            option.setName('value')
                .setDescription('S�?lượt (usage) hoặc gi�?(time)')
                .setRequired(true))
        .addIntegerOption(option =>
            option.setName('maxusers')
                .setDescription('S�?người tối đa có th�?dùng key')
                .setRequired(false))
        .addIntegerOption(option =>
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('key')
                .setDescription('Key đ�?kiểm tra usage')
                .setRequired(true)),

    new SlashCommandBuilder()
        .setName('stats')
        .setDescription('[ADMIN] Xem thống kê s�?dụng bot'),

    new SlashCommandBuilder()
        .setName('logs')
        .setDescription('[ADMIN] Xem lịch s�?s�?dụng bot của user')
        .addStringOption(option =>
            option.setName('userid')
                .setDescription('Discord User ID (đ�?trống đ�?xem của bạn)')
                .setRequired(false))
        .addIntegerOption(option =>
            option.setName('limit')
                .setDescription('S�?lượng logs (mặc định: 10)')
                .setRequired(false)),
    ...
    new SlashCommandBuilder()
        .setName('scan')
        .setDescription('[ADMIN] Quét JobId mới đ�?thêm vào queue'),
];
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        if (commandName === 'createkey') {
            if (!isAdmin(userId)) {
                return await interaction.editReply({ content: '�?Ch�?admin mới có th�?s�?dụng lệnh này!' });
            }
        ...
        if (commandName === 'keyusage') {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: 'Đã s�?dụng', value: `${keyInfo.used}/${keyInfo.value}`, inline: true },
                    { name: 'S�?người dùng', value: `${keyInfo.uniqueUsers}`, inline: true }
                );
        ...
        if (commandName === 'logs') {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`📋 Lịch s�?s�?dụng bot - <@${targetUserId}>`)
                .setColor(0x3498db)
                .addFields(
        ...
        try {
            return await interaction.editReply({ content: '�?Có lỗi xảy ra khi x�?lý lệnh.' });
        } catch (replyError) {
...
client.login(CONFIG.TOKEN).catch(error => {
    console.error('�?Không th�?đăng nhập Discord Bot:', error.message);
